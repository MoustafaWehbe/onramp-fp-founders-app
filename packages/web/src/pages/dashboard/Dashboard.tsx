...
function TodayWorkspace({ startupId }: { startupId: string }) {
  ...
  return <div className="space-y-6">
    <header className="relative overflow-hidden rounded-2xl border border-primary/20 bg-linear-to-br from-primary/11 via-card to-card p-5 sm:p-7">
      <div className="absolute -right-12 -top-20 h-56 w-56 rounded-full bg-primary/10 blur-3xl" />
      <div className="relative flex flex-wrap items-end justify-between gap-5">
        <div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/8 px-3 py-1 text-xs font-medium text-primary"><span className="h-1.5 w-1.5 rounded-full bg-primary" /> Today</div><h1 className="font-display text-2xl font-semibold tracking-tight sm:text-3xl">{greeting()}, {user?.firstName?.trim() || "there"}</h1><p className="mt-2 text-sm text-muted-foreground">Start with the work most likely to move your raise forward.</p></div>
