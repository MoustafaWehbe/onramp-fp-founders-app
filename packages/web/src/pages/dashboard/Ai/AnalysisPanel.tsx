...
import { cancelAiAnalysis, createAiAnalysis, getAiAnalysis, listAiAnalyses, type AiAnalysis, type AiSession } from "../../../lib/ai-api";
...
export function AnalysisPanel({ startupId, session, canCreate, onAskFollowup, onSelectPersona }: Props) {
  ...
  return <section className="mt-6 border-t pt-5"><div className="flex items-center gap-2 text-sm font-medium"><BarChart3 className="h-4 w-4 text-muted-foreground" /> Deck analysis</div><p className="mt-1 text-xs text-muted-foreground">Version {document.versionNumber} · rubric-backed feedback</p>{analysesQuery.isPending ? <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Loading analyses</div> : latest ? <AnalysisState startupId={startupId} analysis={latest} onRetry={() => createMutation.mutate()} onCancel={() => cancelMutation.mutate(latest.id)} pending={createMutation.isPending || cancelMutation.isPending} onAskFollowup={onAskFollowup} onSelectPersona={onSelectPersona} selectedPersonaId={session.persona?.id ?? null} /> : <Button className="mt-3 w-full" size="sm" variant="outline" onClick={() => createMutation.mutate()} disabled={!canCreate || createMutation.isPending}>{createMutation.isPending ? <Loader2 className="animate-spin" /> : <FileSearch />} Analyze this deck</Button>}</section>;
}

function AnalysisState({ startupId, analysis, onRetry, onCancel, pending, onAskFollowup, onSelectPersona, selectedPersonaId }: { startupId: string; analysis: AiAnalysis; onRetry: () => void; onCancel: () => void; pending: boolean; onAskFollowup: (prompt: string) => void; onSelectPersona: (personaId: string | null) => void; selectedPersonaId: string | null }) {
  ...
  return <CompletedAnalysis startupId={startupId} analysis={analysis} onAskFollowup={onAskFollowup} onSelectPersona={onSelectPersona} selectedPersonaId={selectedPersonaId} />;
}

function CompletedAnalysis({ startupId, analysis, onAskFollowup, onSelectPersona, selectedPersonaId }: { startupId: string; analysis: AiAnalysis; onAskFollowup: (prompt: string) => void; onSelectPersona: (personaId: string | null) => void; selectedPersonaId: string | null }) {
