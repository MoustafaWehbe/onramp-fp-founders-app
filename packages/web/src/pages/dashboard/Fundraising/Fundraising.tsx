...
const ROUND_STATUSES: RoundStatus[] = ["draft", "active", "closed", "cancelled"];
const COMMITMENT_STATUSES: CommitmentStatus[] = ["pending", "negotiating", "confirmed", "funded", "withdrawn"];
...
function commitmentTone(status: CommitmentStatus) {
  return status === "funded" ? "bg-success/15 text-success" : status === "confirmed" ? "bg-primary/15 text-primary" : status === "withdrawn" ? "bg-destructive/15 text-destructive" : "bg-muted text-muted-foreground";
}
...
export function Fundraising() {
  ...
  const totals = useMemo(() => {
    const confirmed = commitments.filter((item) => item.status === "confirmed" || item.status === "funded").reduce((sum, item) => sum + (item.amount ?? 0), 0);
    const funded = commitments.filter((item) => item.status === "funded").reduce((sum, item) => sum + (item.amount ?? 0), 0);
    const target = selectedRound?.targetAmount ?? 0;
    return { confirmed, funded, remaining: Math.max(0, target - confirmed), percent: target ? Math.min(100, Math.round((confirmed / target) * 100)) : 0 };
  ...
  return <div className="space-y-6">
    ...
    {rounds.length > 0 && <>
      <div className="grid gap-4 lg:grid-cols-3">{rounds.map((round) => {
        ...
        const raised = isSelected ? totals.confirmed : 0;
        const pct = isSelected ? totals.percent : 0;
        return <button key={round.id} type="button" onClick={() => setActiveRoundId(startupId, round.id)} className={cn("card-elevated p-5 text-left transition-colors hover:border-primary/40", isSelected && "border-primary/60 ring-1 ring-primary/20")}><div className="mb-4 flex items-center justify-between gap-3"><div><div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Round</div><div className="font-display text-lg font-semibold tracking-tight">{round.roundName}</div></div><Badge className={cn("border-0 capitalize", roundTone(round.status))}>{round.status}</Badge></div><div className="mb-1 flex items-end justify-between gap-3"><div className="font-display text-2xl font-semibold tabular-nums">{isSelected ? money(raised, round.currency) : "Select round"}</div><div className="text-xs text-muted-foreground">of {money(round.targetAmount, round.currency)}</div></div><Progress value={pct} className="h-2" /><div className="mt-4 grid grid-cols-3 gap-3 border-t border-border pt-4 text-xs"><Stat label="Min ticket" value={money(round.minimumTicketSize, round.currency)} /><Stat label="Equity" value={round.equityOfferedPercentage === null ? "—" : `${round.equityOfferedPercentage}%`} /><Stat label="Currency" value={round.currency} /></div></button>;
      })}</div>
      {selectedRound && <section className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-display text-xl font-semibold">{selectedRound.roundName}</h2><p className="text-sm text-muted-foreground">Confirmed commitments count toward the target; funded is cash received.</p></div><div className="flex gap-2">{canUpdate && <Button size="sm" variant="outline" onClick={() => setRoundDialog(selectedRound)}><Pencil className="mr-1.5 h-3.5 w-3.5" /> Edit round</Button>}{canCreate && <Button size="sm" onClick={() => setCommitmentDialog("new")} disabled={pipelineQuery.isLoading || (pipelineQuery.data?.length ?? 0) === 0}><Plus className="mr-1.5 h-4 w-4" /> Add commitment</Button>}</div></div><div className="grid gap-4 sm:grid-cols-3"><Metric title="Confirmed" value={money(totals.confirmed, selectedRound.currency)} detail={`${totals.percent}% of target`} /><Metric title="Funded" value={money(totals.funded, selectedRound.currency)} detail="Cash received" /><Metric title="Remaining" value={money(totals.remaining, selectedRound.currency)} detail={`Target ${money(selectedRound.targetAmount, selectedRound.currency)}`} /></div><div className="card-elevated overflow-hidden"><div className="flex items-center justify-between border-b border-border p-5"><div><div className="text-sm font-semibold">Commitments</div><div className="text-xs text-muted-foreground">Track verbal, signed, and funded commitments for this round.</div></div><div className="grid h-9 w-9 place-items-center rounded-md bg-primary/15 text-primary"><TrendingUp className="h-4 w-4" /></div></div><CommitmentTable commitments={commitments} currency={selectedRound.currency} canUpdate={canUpdate} onEdit={setCommitmentDialog} /></div></section>}
    </>}
...
function CommitmentDialog({ commitment, open, pipeline, round, busy, onOpenChange, onCreate, onUpdate }: { commitment: Commitment | "new" | null; open: boolean; pipeline: PipelineEntry[]; round: FundraisingRound | null; busy: boolean; onOpenChange: (open: boolean) => void; onCreate: (input: CommitmentInput) => void; onUpdate: (input: { amount: number; status: CommitmentStatus; expectedCloseDate?: string | null }) => void }) {
  ...
  const [form, setForm] = useState({ pipelineId: "", amount: "", status: "pending" as CommitmentStatus, expectedCloseDate: "" });
  useEffect(() => { setForm(existing ? { pipelineId: existing.pipelineId, amount: String(existing.amount ?? ""), status: existing.status, expectedCloseDate: dateForInput(existing.expectedCloseDate) } : { pipelineId: pipeline[0]?.id ?? "", amount: "", status: "pending", expectedCloseDate: "" }); }, [existing, open, pipeline]);
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>{existing ? "Edit commitment" : "Add commitment"}</DialogTitle><DialogDescription>{existing ? "Update its amount, status, or expected close." : "Commitments are tied to an investor’s deal in this round."}</DialogDescription></DialogHeader><form className="grid gap-4" onSubmit={submit}>{!existing && <Field label="Investor"><select className="h-9 w-full rounded-md border border-border bg-surface/60 px-3 text-sm" value={form.pipelineId} onChange={(e) => setForm({ ...form, pipelineId: e.target.value })}>{pipeline.map((deal) => <option key={deal.id} value={deal.id}>{deal.investor.fullName}{deal.investor.ventureFirm ? ` — ${deal.investor.ventureFirm}` : ""}</option>)}</select></Field>}<div className="grid grid-cols-2 gap-3"><Field label="Amount"><Input type="number" min="0" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} /></Field><Field label="Expected close"><Input type="date" value={form.expectedCloseDate} onChange={(e) => setForm({ ...form, expectedCloseDate: e.target.value })} /></Field></div><Field label="Status"><select className="h-9 w-full rounded-md border border-border bg-surface/60 px-3 text-sm" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as CommitmentStatus })}>{COMMITMENT_STATUSES.map((status) => <option key={status}>{status}</option>)}</select></Field><DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button type="submit" disabled={busy}>{busy ? "Saving…" : "Save commitment"}</Button></DialogFooter></form></DialogContent></Dialog>;
}
...
function Metric({ title, value, detail }: { title: string; value: string; detail: string }) { return <div className="card-elevated p-5"><div className="text-sm font-medium text-muted-foreground">{title}</div><div className="mt-1 font-display text-2xl font-semibold tabular-nums">{value}</div><div className="mt-1 text-xs text-muted-foreground">{detail}</div></div>; }
